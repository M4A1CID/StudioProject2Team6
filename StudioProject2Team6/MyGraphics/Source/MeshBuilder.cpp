...
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float length, bool repeat)
{
	// An array of 3 vectors which represents 3 vertices
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	v.pos.Set(-0.5f,-0.5f,0); v.color = color;
	v.normal.Set(0, 0, 1);
	...
	v.texCoord.Set(500, 0);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f,0.5f,0); v.color = color;
	v.normal.Set(0, 0, 1);

	v.texCoord.Set(500, 500);
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f,0.5f,0); v.color = color;
	v.normal.Set(0, 0, 1);

	v.texCoord.Set(0, 500);
	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateCylinder(const std::string &meshName, Color color, unsigned numStack,unsigned numSlice, float radius, float height)
{
	...
	for (unsigned stack = 0; stack < numStack + 1; ++stack)
	{
		...
		for(unsigned slice = 0; slice < numSlice + 1; ++ slice)
		{
			...
			v.color = color;
			v.normal.Set(cylinderX(theta), 0, cylinderX(theta));
